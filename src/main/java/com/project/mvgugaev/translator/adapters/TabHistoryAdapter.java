...
// Translate history adapter (use in ListView)

public class TabHistoryAdapter extends ArrayAdapter <Tab> implements View.OnClickListener, Filterable {
    ...
    public TabHistoryAdapter(Context context, ArrayList <Tab> tabs) {
    ...
    public void onClick(View v) {
        Tab p = getObject((Integer) v.getTag());
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        Tab item = getObject(position);

        if (item.getFlag().equals("3")) {
            tabButton.setBackgroundResource(R.drawable.ic_to_tabs_picked); //Set picked tab element
        }
        ...
        ((TextView) rowView.findViewById(R.id.text_to)).setText(item.gettTo().length() > 15 ? item.gettTo().substring(0, 10) + ".." : item.gettTo());
    ...
    private Tab getObject(int position) {
